...
-- Replace owner@example.com with your actual admin email before running.
-- This fixes: new row violates row-level security policy for table "orders".
...
-- Give API roles permission to use table. RLS still protects the data.
grant insert on public.orders to anon, authenticated;
grant select on public.orders to authenticated;
grant update on public.orders to authenticated;
grant delete on public.orders to authenticated;

-- Customers/public can place orders.
...
-- Admin can delete test/wrong orders.
...
-- Customer tracking function by phone/order id.
